...
type Props = { momentId: string | null };

export default function MomentClient({ momentId }: Props) {
  ...
  useEffect(() => {
    if (!momentId) return;
    getHistoryForMoment(momentId)
      .then(setHistory)
      .catch((e) => console.error("moment_history write failed", e));
  }, [momentId]);
  ...
  function goTo(nextStep: 0 | 1 | 2 | 3 | 4) {
    ...
    if (momentId) {
      if (step === 1 && nextStep === 2)
        recordWatched(momentId).catch((e) =>
          console.error("moment_history write failed", e)
        );
      if (step === 2 && nextStep === 3)
        recordAsked(momentId).catch((e) =>
          console.error("moment_history write failed", e)
        );
      if (step === 3 && nextStep === 4)
        recordPrayed(momentId).catch((e) =>
          console.error("moment_history write failed", e)
        );
    }
  ...
  // or if the server already recorded it for this moment.
